...
/* ── Hardcoded data (categories, reviews, faqs) ──────────── */
interface Category {
  id: string
  title: string
  subtitle: string
...
function useAnimatedCounter(targetValue: number, duration = 2000) {
  ...
  const startAnimation = useCallback(() => {
    if (startedRef.current || targetValue <= 0) return
...
export function Home() {
  ...
  const { displayValue: animatedCount, startAnimation: startCountAnimation } = useAnimatedCounter(clientCount)
  ...
  useEffect(() => {
    async function fetchClientCount() {
      try {
        ...
        if (typeof data?.clients === 'number' && data.clients > 0) setClientCount(data.clients)
      } catch {
        // fallback: базовое значение 1757
      }
    }
    fetchClientCount()
  }, [])
  ...
  useEffect(() => {
    if (!counterStartedRef.current && clientCount > 0) {
      counterStartedRef.current = true
      startCountAnimation()
    }
  }, [clientCount, startCountAnimation])

  const categories: Category[] = [
    { id: 'certs', title: t('home.cat.certs.title'), subtitle: t('home.cat.certs.subtitle'), count: products.filter(p => p.category === 'certs').length, icon: 'ShieldCheck', color: '#9533ff' },
    { id: 'apps', title: t('home.cat.apps.title'), subtitle: t('home.cat.apps.subtitle'), count: products.filter(p => p.category === 'apps').length, icon: 'Smartphone', color: '#3bb33b' },
    { id: 'utils', title: t('home.cat.utils.title'), subtitle: t('home.cat.utils.subtitle'), count: products.filter(p => p.category === 'utils').length, icon: 'Settings', color: '#fcab14' },
    { id: 'free', title: t('home.cat.free.title'), subtitle: t('home.cat.free.subtitle'), count: products.filter(p => p.category === 'free').length, icon: 'Gift', color: '#25a2e0' },
  ]

  /* Hardcoded reviews as fallback */
  const hardcodedReviews: ReviewItem[] = [
    { id: 1, name: 'Алексей М.', avatar: 'А', rating: 5, text: 'Купил VIP сертификат — работает идеально уже 3 месяца. Все приложения ставятся без проблем, поддержка отвечает моментально.', date: '2 дня назад' },
    { id: 2, name: 'Дарья К.', avatar: 'Д', rating: 5, text: 'Наконец-то нашла нормальный сервис! Раньше покупала на других площадках — через неделю отзывали. Здесь всё стабильно.', date: '5 дней назад' },
    { id: 3, name: 'Максим Р.', avatar: 'М', rating: 4, text: 'Базовый серт на 40 дней — отличный способ попробовать. Потом перешёл на VIP. Всем рекомендую!', date: '1 неделю назад' },
    { id: 4, name: 'Кирилл В.', avatar: 'К', rating: 5, text: 'Топовый магазин. Покупаю тут уже полгода, ни разу не подвели. Цены адекватные, доставка мгновенная.', date: '2 недели назад' },
    { id: 5, name: 'Анна С.', avatar: 'А', rating: 5, text: 'Подруга посоветовала — не пожалела. TikTok без рекламы это просто кайф, а сертификат работает без сбоев.', date: '3 недели назад' },
  ]

  /* Use live reviews if available, else hardcoded */
  const reviews: ReviewItem[] = liveReviews.length > 0
    ? liveReviews.map((r, i) => ({
        id: typeof r.id === 'string' ? i + 100 : Number(r.id),
        name: r.name,
  ...
  const faqs: FaqItem[] = [
    { id: 1, question: t('home.faq.q1'), answer: t('home.faq.a1') },
    { id: 2, question: t('home.faq.q2'), answer: t('home.faq.a2') },
    { id: 3, question: t('home.faq.q3'), answer: t('home.faq.a3') },
    { id: 4, question: t('home.faq.q4'), answer: t('home.faq.a4') },
    { id: 5, question: t('home.faq.q5'), answer: t('home.faq.a5') },
  ]

  const POPULAR_TAGS = [t('home.tag.vipCert'), 'TikTok', 'Scarlet', t('home.tag.free'), 'VK Сова']
  ...
  return (
    <>
      ...
      <section style={{
        ...
        <div className="container" style={{ textAlign: 'center', maxWidth: 800, position: 'relative', zIndex: 1 }}>
          ...
          <motion.div
            ...
            style={{ marginBottom: 12, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}
          >
            {/* Client counter pill */}
            <motion.span
            ...
            >
              ✦ {formatWithSpaces(animatedCount)}+ довольных клиентов
            </motion.span>
          </motion.div>
        ...
        <div className="container">
          <div className="section-head">
            ...
            <Link to="/catalog" className="btn btn-ghost" style={{ fontSize: '0.82rem', gap: 4 }}>
              {t('home.allCategories')} <ArrowRight size={14} />
            </Link>
          </div>
                ...
                <Link to={`/catalog?category=${cat.id}`}>
                  <div className="card card-hover" style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <h3 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: 3, color: '#fff' }}>{cat.title}</h3>
                      <p style={{ fontSize: '0.8rem', color: 'var(--text-3)' }}>{cat.subtitle}</p>
                    </div>
                    <div style={{
        ...
        <div className="container">
          ...
          <div style={{ textAlign: 'center', marginTop: 'var(--sp-8)' }}>
            <Link to="/catalog" className="btn btn-gradient" style={{ padding: '14px 32px', gap: 6 }}>
              {t('home.fullCatalog')} <ArrowRight size={16} />
            </Link>
          </div>
        </div>
      </section>
      ...
      <section id="why" className="section">
        <div className="container">
          <h2 style={{ textAlign: 'center', marginBottom: 'var(--sp-8)' }}>{t('home.whyUs')}</h2>
          ...
          <div style={{
            ...
            {[
              ...
              { icon: <Shield size={24} />, title: t('home.why.guarantee'), desc: t('home.why.guaranteeDesc') },
            ...
            {reviews.slice(0, 4).map((review, i) => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  {review.verified && (
                    <span style={{
                      ...
                      background: 'rgba(59,179,59,0.08)',
                      border: '1px solid rgba(59,179,59,0.2)',
                      fontSize: '0.68rem', fontWeight: 600, color: '#3bb33b',
                      whiteSpace: 'nowrap',
                    }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    <p style={{ fontSize: '0.82rem', fontWeight: 600 }}>{review.name}</p>
          ...
          >
            ...
            <div style={{
              ...
              {[
                ...
                { label: 'iPad', models: `Air / Pro / Mini / ${t('home.compat.ipadBasic')}`, icon: <Tablet size={22} style={{ color: '#06b6d4' }} /> },
                { label: 'iPod touch', models: t('home.compat.ipodGen'), icon: <Music size={22} style={{ color: '#f59e0b' }} /> },
              ].map(d => (
